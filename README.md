...
Данный проект - полноценный веб-сайт, разработанный для взаимодействия с блогами, где вы можете сохранять блоги, создавать и оценивать. На данном сайте реализована система аутентификации с JWT токеном и cookie - файлами, базовые CRUD - операции с базой данных, динамические страницы отображающие данные из базы и взаимодействующие между собой frontend и backend. Использовался паттерн проектирования MVC(Model - View - Controller) который является составной частью Spring web. Также в проекте реализована система верификации аккаунта через почту, используя AWS (Amazon web services) SES (Simple email service).
...
Некоторые файлы, такие как EmailChecker, applciation.yaml были скрыты, так как там имеются данные по AWS Client, на которые ругается GitHub protection
...
+ AWS SES: Сервис от Amazon, которая предоставляет почтовый сервис для рассылки
...
+ Верификация по почте: Когда пользователь регистриуется, он указывает свою почту, и данную почту мы добавляем в Amazon SES, для дальнейшей работы с кодом подтверждения.
+ Восстановление пароля: В странице входа в аккаунт есть рабочий раздел: "Забыли пароль", в которой пользователь может восстановть/изменить свой пароль используя почту для подтверждения.
